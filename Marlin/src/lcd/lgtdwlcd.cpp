...
{
	if (!all_axes_trusted() && !all_axes_homed())
		return;
	...
	DEBUG_ECHOLN("save X:", resume_x_position);
	DEBUG_ECHOLN("save Y:", resume_y_position);
	DEBUG_ECHOLN("save E:", resume_e_position);
	DEBUG_ECHOLN("save feedrate", feedrate_mm_s);
	...
	planner.synchronize();
	DEBUG_ECHOPGM_P("queue-length", queue.length);
}
...
{
	DEBUG_ECHOLN("ADDR: ", Rec_Data.addr);
	DEBUG_ECHOLN(" VALUE:", Rec_Data.data[0]);
	uint16_t LGT_feedrate = 0;
	...
	// ----- print home menu -----
		...
		case eBT_PRINT_HOME_RESUME:
			...
			if (all_axes_trusted() || all_axes_homed()) {
				...
				DEBUG_ECHOLN("cur feedrate", feedrate_mm_s);
	...
	// ----- manual leveling menu -----
		case eBT_UTILI_LEVEL_CORNER_POS_1:
				...
				if (xy_home == false)
				{
					...
					queue.enqueue_one(PSTR("G28 X0 Y0"));
					xy_home = true;
				}
				queue.enqueue_one(PSTR("G1 X50 Y50"));
				...
				queue.enqueue_one(PSTR("G1 Z10 F500"));
				queue.enqueue_one(PSTR("G1 X50 Y50 F5000"));
				queue.enqueue_one(PSTR("G1 Z0 F300"));		
			#else  //LK4_PRO
			...
			// 	break;
				...
				queue.enqueue_one(PSTR("G1 Z10 F500"));
				queue.enqueue_one(PSTR("G1 X30 Y30 F5000"));
				queue.enqueue_one(PSTR("G1 Z0 F300"));
			#endif
			break;
		case eBT_UTILI_LEVEL_CORNER_POS_2: //45 002D
				...
				queue.enqueue_one(PSTR("G1 X250 Y50"));
			#elif defined(LONGER_LK5)
				if (xyz_home == false)
				{
					#if ENABLED(COOL_BEFORE_LEVELING)
				...
				queue.enqueue_one(PSTR("G1 Z10 F500"));
				queue.enqueue_one(PSTR("G1 X250 Y50 F5000"));
				queue.enqueue_one(PSTR("G1 Z0 F300"));				
			#else  //LK4_PRO
				if (xyz_home == false)
				{
				...
				queue.enqueue_one(PSTR("G1 Z10 F500"));
				queue.enqueue_one(PSTR("G1 X190 Y30 F5000"));
				queue.enqueue_one(PSTR("G1 Z0 F300"));
			#endif
			break;
		case eBT_UTILI_LEVEL_CORNER_POS_3:
				...
				queue.enqueue_one(PSTR("G1 X250 Y250"));
			#elif defined(LONGER_LK5)
				if (xyz_home == false)
				{
					#if ENABLED(COOL_BEFORE_LEVELING)
				...
				queue.enqueue_one(PSTR("G1 Z10 F500"));
				queue.enqueue_one(PSTR("G1 X250 Y250 F5000"));
				queue.enqueue_one(PSTR("G1 Z0 F300"));					
			#else  //LK4_PRO
				if (xyz_home == false)
				{
				...
				queue.enqueue_one(PSTR("G1 Z10 F500"));
				queue.enqueue_one(PSTR("G1 X190 Y190 F5000"));
				queue.enqueue_one(PSTR("G1 Z0 F300"));
			#endif
			break;
		case eBT_UTILI_LEVEL_CORNER_POS_4:
				...
				queue.enqueue_one(PSTR("G1 X50 Y250"));		
			#elif defined(LONGER_LK5)
				if (xyz_home == false)
				{
					#if ENABLED(COOL_BEFORE_LEVELING)
				...
				queue.enqueue_one(PSTR("G1 Z10 F500"));
				queue.enqueue_one(PSTR("G1 X50 Y250 F5000"));
				queue.enqueue_one(PSTR("G1 Z0 F300"));				
			#else  //LK4_PRO
				if (xyz_home == false)
				{
				...
				queue.enqueue_one(PSTR("G1 Z10 F500"));
				queue.enqueue_one(PSTR("G1 X30 Y190 F5000"));
				queue.enqueue_one(PSTR("G1 Z0 F300"));  
			#endif
			break;
		case eBT_UTILI_LEVEL_CORNER_POS_5:
				...
				queue.enqueue_one(PSTR("G1 X150 Y150"));			
			#elif defined(LONGER_LK5)
				if (xyz_home == false)
				{
					#if ENABLED(COOL_BEFORE_LEVELING)
				...
				queue.enqueue_one(PSTR("G1 Z10 F500"));
				queue.enqueue_one(PSTR("G1 X150 Y150 F5000"));
				queue.enqueue_one(PSTR("G1 Z0 F300"));				
			#else  //LK4_PRO
				if (xyz_home == false)
				{
				...
				queue.enqueue_one(PSTR("G1 Z10 F500"));
				queue.enqueue_one(PSTR("G1 X110 Y110 F5000"));
				queue.enqueue_one(PSTR("G1 Z0 F300"));
			#endif
			break;
		case eBT_UTILI_LEVEL_CORNER_BACK:
				...
				if (xy_home) {
					xy_home = false;
					queue.enqueue_one(PSTR("G1 Z10 F500"));	//up 10mm to prevent from damaging bed
				}
			#else //LK4_PRO or LONGER_LK5
				if (xyz_home) {
					xyz_home = false;
					queue.enqueue_one(PSTR("G1 Z10 F500"));	//up 10mm to prevent from damaging bed
...
void LGT_SCR_DW::LGT_Display_Filename()
{
    ...
    const uint16_t FileCnt = card.get_num_items();  //FileCnt:Total number of files
	DEBUG_ECHOLN("gcode cout: ", FileCnt);
...
void LGT_SCR_DW::LGT_Stop_Printing()
{
	...
	if (is_recovery_resuming) {
		...
		queue.enqueue_one(PSTR("G1 Z10"));
		queue.enqueue_one(PSTR("G90"));
		queue.enqueue_one(PSTR("G28 X0"));
	}

	queue.enqueue_one(PSTR("M2000"));	// goto home page
}
...
static int8_t errorIndex(const char *error, const char *component)
{
	...
	if (IS_ERROR(MSG_ERR_MINTEMP)) {
		...
		if (IS_E1())
			...
			index = -1;
	} else if (IS_ERROR(MSG_ERR_HEATING_FAILED)) {
		if (IS_E1())
			index = 4;
		else if (IS_BED())
			index = 5;
			...
			index = -1;
	} else if (IS_ERROR(MSG_ERR_THERMAL_RUNAWAY)) {
			if (IS_E1())
			index = 6;
		else if (IS_BED())
			index = 7;
